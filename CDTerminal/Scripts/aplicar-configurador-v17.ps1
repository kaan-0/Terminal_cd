...
$payload = Join-Path $root "Payload_V17\Components\Pages"
...
foreach ($name in @("Inicio.razor", "Inicio.razor.css")) {
    if (-not (Test-Path (Join-Path $payload $name))) {
        throw "No se encontro el archivo V17: $name"
    }
}

$stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$backupDir = Join-Path $root "Backups\antes-configurador-v17-$stamp"
...
Write-Host "Configurador IoT V17 Produccion HTTPS aplicado." -ForegroundColor Green
